...
import { withTx, type AnyDb } from "@/lib/db/types";
...
/** Options for {@link processActivity}; `afterUpsert` exists so tests can inject a failing post-write step. */
export interface ProcessOptions {
  afterUpsert?: (dbc: AnyDb, activityId: string) => Promise<void>;
}
...
export function processActivity(
  ...
  // One transaction so a failure in the lap prune or in `afterUpsert` cannot leave
  // an activity half-rewritten (neon-serverless/PGlite both support real transactions).
  return withTx(dbc, async (tx) => {
    ...
    if (n.laps.length) {
      ...
      await tx.delete(laps).where(eq(laps.activityId, activityId));
    }

    await (opts.afterUpsert ?? afterUpsert)(tx, activityId);
