...
| **Trends** | Am I getting fitter — and what should I do about it? | Range chips (4w/8w/12w/block/season). **Today's read** hero: a coach-written daily analysis (regenerated each morning and after every sync) tying the numbers together and naming the one thing to act on. Then six **question tiles**, each with a plain-words headline, a delta and the chart as evidence: *Am I fitter aerobically?* (pace at easy HR over time + long-run drift) · *Am I training too hard?* (easy/hard share per week vs 80/20 line) · *How much am I running?* (weekly km by zone) · *Am I absorbing the load?* (fitness/fatigue/form + acute:chronic ramp with the 0.8–1.3 band) · *Am I consistent?* (12-week daily heatmap, adherence %) · *Am I on track for the goal?* (predicted goal-race time week by week vs target, benchmarks marked). Tap a tile → deep-dive with the full chart and explanation |
