...
| `brief` | "Brief me" on Session / Home (day-of, cached per session) | `{summary, target_paces, hr_ceiling, focus, warmup, if_then[]}` — refreshes `tips` with today's form, yesterday's load and the athlete's note |
