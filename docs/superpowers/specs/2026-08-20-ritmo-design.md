...
| **Calendar** (one component, two entry points: Plan → Month toggle, Runs → calendar button) | Browse past runs and the plan by month               | Month grid, Monday start; each day shows a filled dot per completed run (type colour) and an outline dot per planned session, so plan-vs-actual is visible at a glance; today ringed; tap a day → that day's run card(s) and/or planned session below, with month totals. Arrows/swipe between months; from Plan the selection opens Session, from Runs it opens Run detail                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Trends**                                                                                  | Am I getting fitter — and what should I do about it? | Range chips (4w/8w/12w/block/season). **Today's read** hero: a coach-written daily analysis (regenerated each morning and after every sync) tying the numbers together and naming the one thing to act on. Then six **question tiles**, each with a plain-words headline, a delta and the chart as evidence: _Am I fitter aerobically?_ (pace at easy HR over time + long-run drift) · _Am I training too hard?_ (easy/hard share per week vs 80/20 line) · _How much am I running?_ (weekly km by zone) · _Am I absorbing the load?_ (fitness/fatigue/form + acute:chronic ramp with the 0.8–1.3 band) · _Am I consistent?_ (12-week daily heatmap, adherence %) · _Am I on track for the goal?_ (predicted goal-race time week by week vs target, benchmarks marked). Tap a tile → deep-dive with the full chart and explanation |
...
(`plan_block`|`plan_week`|`tips`|`debrief`|`brief`|`daily_read`|`review`), requested_at,
athlete_note, context_json, response_json, model, prompt_version,
tokens_in, tokens_out, status, error

**daily_load** — athlete_id, date, load, fitness, fatigue, form, acwr,
pace_at_ref_hr_s (nullable), hard_share_7d, goal_prediction_s (nullable)
...
- **Pace at easy HR**: for each easy run, pace normalised to the athlete's
  reference HR (default: top of Z2) using the run's own pace–HR slope across
  laps; 7-run rolling median charted. The headline aerobic-fitness number —
  "5:52 /km at 145 bpm", shown instead of a unitless efficiency index.
- **Intensity distribution**: share of moving time in Z1–2 vs Z3+ per week,
  against an 80/20 target; also flags easy runs whose avg HR sits in Z3.
- **Acute:chronic ramp**: 7-day load ÷ 28-day load; 0.8–1.3 shown as the safe
  band, >1.5 raises a coach warning.
- **Goal prediction trend**: the Riegel/VDOT prediction recomputed weekly and
  stored, so Trends can show its trajectory against the goal time.
...
| `daily_read` | Cron each morning (06:30 local) and after each sync         | `{headline, paragraph, watch_item, numbers{fitness_delta, form, hard_pct, goal_prediction}}` — ~80 words tying today's metrics together: what improved, the one thing to watch, what to do today. Shown as the Trends hero and as a card on Home when new. Cheap (Sonnet, ~3 k tokens)                                       |
...
needed. The validator runs on the _edited_ week: target days that would violate
...
- **Discipline over heroics**: _the plan is the goal_. Praise running to the
...
- Vercel Cron: nightly sync 03:00 Europe/Lisbon; daily read 06:30; weekly
  review Sunday 20:00.
