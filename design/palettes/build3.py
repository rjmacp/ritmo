...
BG,TILE,TXT,MUT,GA,GB,ACC="#ffffff","#f3f4f6","#141a26","#5f6776","#16223d","#2c4570","#16223d"
...
for i,(n,STL,AMB,GRN,STL_H,AMB_H,GRN_H,STL_T,AMB_T,GRN_T,note) in enumerate(V):
    ...
    for scr in ("Main","Trends"):
        out=f'M{n}{scr}.dc.html'; shutil.copy(os.path.join(d,f'{scr}.dc.html'),out)
        arts.append({"file":out,"title":f"Midnight · {n} · {'Home' if scr=='Main' else 'Trends'}","x":i*470,"y":4200+(0 if scr=='Main' else 900),"w":390,"h":844})
    notes.append({"id":f"m{i+1}","x":i*470,"y":4100,"w":390,"text":note})
c=json.load(open('canvas.json'))
c["artboards"]=[a for a in c["artboards"] if not a["file"].startswith("M") or a["file"]=="Main.dc.html"]+arts
c["annotations"]=[a for a in c["annotations"] if not a["id"].startswith("m")]+notes
