...
- **Auth** — Resend magic-link sign-in, gated to one allow-listed athlete. Every page and
  API route that needs a session calls `requireAthlete()`; the sign-in form gives the same
  answer for any address, so it cannot be used to tell which one is registered.
